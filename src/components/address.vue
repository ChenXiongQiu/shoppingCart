<template>
    ...
    <div>
        ...
        <div class="container">
            <div class="checkout-addr">
                ...
                <div class="addr-list-wrap">
                    <div class="addr-list">
                        <ul>
                            <li v-for="(address,index) in filteAddress" :class="{'check':addressIndex==index}" @click="addressIndex=index">
                                <dl>
                                    <dt v-text="address.userName"></dt>
                                    <dd class="address" v-text="address.streetName"></dd>
                                    <dd class="tel" v-text="address.tel"></dd>
                                </dl>
                                <div class="addr-opration addr-edit"  @click="modify(address,index)">
                                ...
                                <div class="addr-opration addr-del">
                                    <a href="javascript:;" class="addr-del-btn" @click="addrDel(address,index)">
                                ...
                                <div class="addr-opration addr-set-default" v-show="!address.isDefault">
                                    <a href="javascript:;" class="addr-set-default-btn" @click="setDefaultAddress(address)"><i>设为默认</i></a>
                                </div>
                                <div class="addr-opration addr-default" v-show="address.isDefault">默认地址</div>
                                <div class="md-modal modal-msg md-modal-transition" id="showModal" :class="{'md-show':isModify}" v-show="address.isShow">
                                    <div class="md-modal-inner">
                                        <div class="md-top">
                                            <button class="md-close" @click="cancel">关闭</button>
                                        </div>
                                        <div class="md-content">
                                            <div class="confirm-tips my-p">
                                                <p id="cusLanInfo" lan="Cart.Del.Confirm">{{doTitle}}?</p>
                                                <input type="text" placeholder="用户名" v-model="address.userName">
                                                <input type="text" placeholder="地址" v-model="address.streetName">
                                                <input type="text" placeholder="电话" v-model="address.tel">                       
                                            </div>
                                            <div class="btn-wrap col-2">
                                                <button class="btn btn--m my-btn" id="btnModalConfirm" @click="modifyOver()">Yes</button>
                                                <button class="btn btn--m btn--red my-btn" id="btnModalCancel" @click="cancel">No</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>                               
                            </li>
                            <li class="addr-new" @click="addrNew">
        ...
        <div class="md-modal modal-msg md-modal-transition" id="showModal" :class="{'md-show':isWrite}">
            <div class="md-modal-inner">
                <div class="md-top">
                    <button class="md-close" @click="cancel">关闭</button>
                </div>
                <div class="md-content">
                    <div class="confirm-tips my-p">
                        ...
                        <input type="text" placeholder="用户名" v-model="newAddress.userName">
                        <input type="text" placeholder="地址" v-model="newAddress.streetName">
                        <input type="text" placeholder="电话" v-model="newAddress.tel">                       
                    </div>
                    <div class="btn-wrap col-2">
                        <button class="btn btn--m my-btn" id="btnModalConfirm" @click="addOver">Yes</button>
                        <button class="btn btn--m btn--red my-btn" id="btnModalCancel" @click="cancel">No</button>
                    </div>
                </div>
            </div>
        </div>
        <div class="md-overlay" id="showOverLay" v-show="isWrite"></div>       
        <div class="md-overlay" id="showOverLay" v-show="isModify"></div>       
    </div>
</template>
<script>
